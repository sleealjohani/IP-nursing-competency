...
export function NursePortal({lang}:{lang:Lang}){
  ...
  function go(pos:number){setPosition(pos);setSaveState('idle');scrollTo({top:0,behavior:'smooth'})}
  ...
  if(view==='locked')return <GlassCard className="locked-card"><BadgeCheck size={34}/><h1>{rtl?'تم استلام نماذجك.':'Your forms have been received.'}</h1><p>{rtl?'تم إرسال كل نموذج باسمك إلى الإدارة، والتقييم مقفل الآن بانتظار مراجعة المقيم.':'Each form was sent to the administration under your name. The assessment is locked and awaiting evaluator review.'}</p>{message&&<div className={`notice ${message.type}`}>{message.text}</div>}<div className="metrics"><Metric value={session.nurse?.name||'—'} label={t.name}/><Metric value={<span dir="ltr">{session.nurse?.job_number}</span>} label={t.jobNumber}/><Metric value={forms.length} label={rtl?'نموذج':'Forms'}/><Metric value={<StatusBadge status={session.status||'submitted'}/>} label={t.status}/></div><button onClick={leave}>{rtl?'العودة لبداية النظام':'Return to start'}</button></GlassCard>
  if(view==='review')return <>{message&&<div className={`notice ${message.type}`}>{message.text}</div>}<ReviewView lang={lang} answers={answers} busy={busy} onBack={()=>{setView('assessment')}} onOpen={pos=>{go(pos);setView('assessment')}} onSubmit={()=>void submit()}/></>
