...
import { CATEGORY_LABEL, forms, questions } from '../../lib/content'
...
export function ReviewView(p:Props){
  ...
  return <div className="question-wrap"><GlassCard>
    ...
    <h1>{rtl?'راجع نماذجك قبل الإرسال':'Review your forms before submitting'}</h1>
    <div className="metrics"><Metric value={`${answered}/${questions.length}`} label={t.progress}/><Metric value={`${doneForms}/${forms.length}`} label={rtl?'نماذج مكتملة':'Forms complete'}/><Metric value={Object.values(p.answers).filter(x=>x==='M').length} label="M — Met"/><Metric value={Object.values(p.answers).filter(x=>x==='NM').length} label="NM — Not Met"/></div>
    ...
    <div className="review-list">{forms.map((f,i)=>{
      ...
      return [head,<button key={f.id} className={s.result==='Incomplete'?'missing':''} onClick={()=>p.onOpen(first.index)}><span>{i+1}</span><span><b dir="ltr">{f.title}</b><small>{s.answered}/{s.total}</small></span><strong dir="ltr">{s.result==='Incomplete'?'—':f.scale==='equipment'?'✓':percentLabel(s.percent)}</strong></button>]
    })}</div>
