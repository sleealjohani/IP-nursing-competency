...
import { Activity, BadgeCheck, FileArchive, LogOut, Search, Settings, ShieldCheck, Trash2 } from 'lucide-react'
...
export function ManagerPortal({lang}:{lang:Lang}){
  ...
  async function deleteSession(s:SessionRow){const n=data.nurses.find(x=>x.id===s.nurse_id);const name=n?`${n.name} (${n.job_number})`:'';if(!confirm(rtl?`حذف المشارك ${name} وجميع نماذجه وإجاباته واعتماداته نهائيًا؟ لا يمكن التراجع عن هذا الإجراء.`:`Permanently delete ${name} with all their forms, answers and approvals? This cannot be undone.`))return;setWorking('delete');setMessage('');setNotice('');const {data:res,error}=await supabase.rpc('staff_delete_session',{p_session:s.id});setWorking('');const d=res as {ok?:boolean;error?:string}|null;if(error||!d?.ok){setMessage(error?.message||d?.error||'Error');return}if(selected===s.id)setSelected(null);setNotice(rtl?`تم حذف ${name}.`:`${name} deleted.`);await refresh()}
  ...
  return <div className="manager-layout">
    ...
    <div className="manager-content">{message&&<div className="notice error">{message}</div>}{tab==='evaluator'?<EvaluatorPanel lang={lang} userId={user.id} profile={evaluator} onSaved={setEvaluator}/>:<>
      ...
      <GlassCard><div className="filter-row"><label className="search-box"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t.search}/></label><select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">{t.all}</option><option value="in_progress">in_progress</option><option value="submitted">submitted</option><option value="reopened">reopened</option><option value="completed">completed</option></select></div><div className="session-table"><div className="table-head"><span>{t.nurses}</span><span>{t.status}</span><span>{rtl?'بدأ':'Started'}</span><span>{t.actions}</span></div>{loading?<Spinner/>:filtered.length===0?<p className="empty">{t.noData}</p>:filtered.map(s=>{const n=data.nurses.find(x=>x.id===s.nurse_id);const count=data.answers.filter(a=>a.session_id===s.id).length;return <div className="table-row" key={s.id}><span><b>{n?.name||'—'}</b><small dir="ltr">{n?.job_number}</small></span><span><StatusBadge status={s.status}/><small dir="ltr">{count}/{questions.length}</small></span><span dir="ltr">{new Date(s.started_at).toLocaleDateString('en-GB')}</span><span className="row-actions"><button onClick={()=>setSelected(s.id)}>{t.view}</button><button className="icon-btn danger" disabled={!!working} title={t.deleteParticipant} aria-label={t.deleteParticipant} onClick={()=>void deleteSession(s)}><Trash2 size={16}/></button></span></div>})}</div></GlassCard>
    </>}</div>
    {selectedSession&&selectedNurse&&<SessionPanel lang={lang} session={selectedSession} nurse={selectedNurse} answers={data.answers.filter(a=>a.session_id===selectedSession.id)} reviews={data.reviews.filter(r=>r.session_id===selectedSession.id)} evaluator={evaluator} onDelete={()=>void deleteSession(selectedSession)} onClose={()=>setSelected(null)} onRefresh={async()=>{await refresh();}}/>}
  </div>
}
