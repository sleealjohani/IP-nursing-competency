...
import { Activity, LogOut, Search, Settings, ShieldCheck, Users } from 'lucide-react'
...
export function ManagerPortal({lang}:{lang:Lang}){
  ...
  const [content,setContent]=useState<ClinicalContent|null>(null),[data,setData]=useState<Dashboard>({sessions:[],nurses:[],answers:[],reviews:[]})
  const [loading,setLoading]=useState(true),[message,setMessage]=useState(''),[search,setSearch]=useState(''),[status,setStatus]=useState('all'),[selected,setSelected]=useState<string|null>(null),[tab,setTab]=useState<'dashboard'|'evaluator'>('dashboard')
  ...
  async function boot(){setLoading(true);setContent(await loadClinicalContent());const {data:{session}}=await supabase.auth.getSession();setAuth(session);setUser(session?.user||null);if(session)await loadStaff(session.user);else setLoading(false)}
  ...
  if(!content)return <GlassCard><Spinner/></GlassCard>
  ...
  return <div className="manager-layout">
    ...
    <div className="manager-content">{message&&<div className="notice error">{message}</div>}{tab==='evaluator'?<EvaluatorPanel lang={lang} userId={user.id} profile={evaluator} onSaved={setEvaluator}/>:<>
      <div className="page-title"><div><div className="eyebrow"><Activity size={14}/>{t.dashboard}</div><h1>{rtl?'مركز متابعة كفاءات التمريض':'Nursing Competency Control Center'}</h1></div><button onClick={()=>void refresh()}>{rtl?'تحديث':'Refresh'}</button></div>
      ...
      <GlassCard><div className="filter-row"><label className="search-box"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t.search}/></label><select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">{t.all}</option><option value="in_progress">in_progress</option><option value="submitted">submitted</option><option value="reopened">reopened</option><option value="completed">completed</option></select></div><div className="session-table"><div className="table-head"><span>{t.nurses}</span><span>{t.status}</span><span>{rtl?'بدأ':'Started'}</span><span>{t.actions}</span></div>{loading?<Spinner/>:filtered.length===0?<p className="empty">{t.noData}</p>:filtered.map(s=>{const n=data.nurses.find(x=>x.id===s.nurse_id);const count=data.answers.filter(a=>a.session_id===s.id).length;return <div className="table-row" key={s.id}><span><b>{n?.name||'—'}</b><small dir="ltr">{n?.job_number}</small></span><span><StatusBadge status={s.status}/><small>{count}/{content.questions.length}</small></span><span dir="ltr">{new Date(s.started_at).toLocaleDateString('en-GB')}</span><span><button onClick={()=>setSelected(s.id)}>{t.view}</button></span></div>})}</div></GlassCard>
    </>}</div>
    {selectedSession&&selectedNurse&&<SessionPanel lang={lang} content={content} session={selectedSession} nurse={selectedNurse} answers={data.answers.filter(a=>a.session_id===selectedSession.id)} reviews={data.reviews.filter(r=>r.session_id===selectedSession.id)} evaluator={evaluator} onClose={()=>setSelected(null)} onRefresh={async()=>{await refresh();}}/>}
  </div>
}
